...
- **판정이 흔들리면 프롬프트 규칙을 늘리기 전에 입력을 의심한다.** 진술대조권에서 규칙을 8개 → 3개로 줄였는데 성적이 올랐고, 판정을 바꾼 것은 입력 수정 세 개였다 — 대상 발화만 주기 · 시점 컷오프를 코드로 · `phase` 붙이기 (→ [프롬프트 §2-3](log_analysis/prompts/진술대조.md)). **서버가 아는 사실을 안 주면 모델이 추측하고, 추측은 과제 방향으로 기운다.**
