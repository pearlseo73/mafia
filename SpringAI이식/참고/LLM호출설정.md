...
**조간신문은 아직 안 재봤다.** 위 값들은 출력이 짧은 판정·추출이라 500자 창작에 그대로 쓸 수 없다. **생성 창이 R1 4초 / R2 9초뿐이라 이 값이 재시도 여부를 정한다** → [조간신문 §4](조간신문.md). `write_newspaper()`가 `sec`를 돌려주므로 R1·R2·R3 3회 실행에서 함께 받는다.
...
> **2026-07-31 정정.** 아래는 `.entity()` + `s.text(...)` 로 적혀 있었다. **둘 다 쓰지 않는다** — `.entity()` 는 프롬프트를 바꿔버리고(CLAUDE.md), SYSTEM 을 템플릿으로 렌더하면 JSON 예시가 깨진다(아래 `.st` 절).

```java
String raw = chatClient.prompt()
        .system(SYS.getContentAsString(StandardCharsets.UTF_8))   // 문자열. 렌더 없음
        .user(u -> u.text(USER_PROMPT, StandardCharsets.UTF_8)
                .param("target", target)
                .param("utterances", lines))
        .options(OpenAiChatOptions.builder().model("gpt-4o-mini").maxTokens(512))
        .call()
        .content();                                               // 파싱은 직접
```
...
- **`.content()` 로 받아 직접 파싱하고 실패는 폴백으로 흡수한다.** ` ```json ` 펜스를 벗기는 처리가 필요하다
...
### 🔴 SYSTEM `.st`는 렌더하지 않는다 — USER 만 렌더한다 (2026-07-31 확정)

`.st`는 StringTemplate 파일이고 **`{}`가 변수 구분자다.** 그런데 **출력 형식 지시로 넣은 JSON 예시가 SYSTEM 프롬프트 안에 있다.**

> 전에는 "출력 형식 지시를 `.st`에 쓰지 않고 `BeanOutputConverter`에 맡긴다"로 적혀 있었다. **그 방향을 버렸다** — `BeanOutputConverter`(`.entity()`)는 스키마 지시문을 프롬프트 뒤에 자동으로 덧붙여 노트북과 다른 프롬프트를 보내게 된다(CLAUDE.md). 형식 지시는 프롬프트에 직접 적고 파싱을 직접 한다.

노트북 프롬프트 10개를 실제로 세어본 결과다.

| SYSTEM 파일 | 중괄호 |
|---|---|
| `statement-compare-system.st` | 2쌍 — `{"hasFindings": true, ...}` 등 |
| `keyword-extract-system.st` | 2쌍 — `{"keywords": [{...}]}` |
| `title-award-system.st` | 2쌍 — `{"titles": [{...}]}` |
| `ai-judge-system.st` | 1쌍 — `처형 대상은 {닉네임}입니다.` |
| `newspaper-system.st` | 없음 (유일) |

**렌더하면 JSON 예시가 사라지고, 그러면 모델이 JSON을 내놓지 않는다.** 예외가 아니라 조용히 품질만 떨어지는 실패다. `ai-judge`는 마지막 문장 형식 지시가 사라져 파싱이 실패한다.

```java
// SYSTEM — 문자열로 읽어 String 오버로드에 넘긴다. 템플릿을 거치지 않는다
.system(SYS.getContentAsString(StandardCharsets.UTF_8))

// USER — 이쪽만 템플릿. 중괄호가 전부 실제 변수다
.user(u -> u.text(USER_RES, StandardCharsets.UTF_8).param("log", log).param("target", target))
```

**노트북도 정확히 같은 경계다** — 각 프롬프트 셀에 `SYSTEM 에 .format() 을 쓰지 않는다. 값을 끼우는 것은 USER 쪽뿐이다`라고 적혀 있다. 이식할 때 이 경계를 그대로 옮긴다.

USER 파일 5개의 중괄호는 전부 실제 변수이므로 그대로 `.param()` 이름이 된다 → **SpringAI이식/00-코드스타일 §6**

> SYSTEM 도 템플릿으로 넣고 싶으면 렌더러 구분자를 바꾸는 방법이 있다(`StTemplateRenderer`의 `startDelimiterToken`). **권하지 않는다** — 프롬프트 문안을 건드리게 되고, 노트북과 다른 문자열을 보내는 순간 실측 성적이 무의미해진다.
